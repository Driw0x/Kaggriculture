...
from src.learning.target_encoder import PURCHASE_NAMES, occurrence_names, quantity_names, sell_names
...
def compute_losses(output, batch):
    hire = F.smooth_l1_loss(output["hire"], batch["hire"])
    purchase_bundle = F.cross_entropy(output["purchase_bundle"], batch["purchase_bundle"])
    occurrence = F.binary_cross_entropy_with_logits(output["occurrence"], batch["occurrence"])
    quantity = masked_smooth_l1(output["quantity"], batch["quantity"], batch["quantity_mask"])
    sell = masked_smooth_l1(output["sell_ratio"], batch["sell_ratio"], batch["sell_mask"])
    total = hire + purchase_bundle + occurrence + quantity + sell
...
def create_stats():
    return {
        "samples": 0,
        "loss": {"total": 0.0, "hire": 0.0, "purchase_bundle": 0.0, "occurrence": 0.0, "quantity": 0.0, "sell": 0.0},
        "hire_abs_error": 0.0,
        "purchase_bundle_correct": 0,
...
def update_metrics(stats, output, batch):
    ...
    stats["hire_abs_error"] += torch.abs(output["hire"] - batch["hire"]).sum().item()

    purchase_prediction = output["purchase_bundle"].argmax(dim=1)
    stats["purchase_bundle_correct"] += int((purchase_prediction == batch["purchase_bundle"]).sum())

    prediction = torch.sigmoid(output["occurrence"]) >= 0.5
    target = batch["occurrence"] > 0.5
    stats["tp"] += (prediction & target).sum(dim=0).cpu()
    stats["fp"] += (prediction & ~target).sum(dim=0).cpu()
    stats["fn"] += (~prediction & target).sum(dim=0).cpu()
...
def finalize_metrics(stats):
    ...
    return {
        ...
        "purchase_bundle": stats["loss"]["purchase_bundle"] / samples,
        "occurrence": stats["loss"]["occurrence"] / samples,
        "quantity": stats["loss"]["quantity"] / samples,
        "sell": stats["loss"]["sell"] / samples,
        "hire_mae_log": stats["hire_abs_error"] / samples,
        "purchase_bundle_acc": stats["purchase_bundle_correct"] / samples,
...
def run_epoch(model, loader, device, optimizer=None):
    ...
    for batch in loader:
        ...
        for name, loss in losses.items():
            stats["loss"][name] += loss.item() * batch_size
...
def print_metrics(name, metrics):
    print(
        f"{name:<5} loss={metrics['total']:.4f} hire={metrics['hire']:.4f} "
        f"bundle={metrics['purchase_bundle']:.4f} occ={metrics['occurrence']:.4f} "
        f"qty={metrics['quantity']:.4f} sell={metrics['sell']:.4f} "
        f"hire_mae_log={metrics['hire_mae_log']:.3f} "
        f"bundle_acc={100 * metrics['purchase_bundle_acc']:.2f}% "
        f"occ_micro_f1={metrics['occ_micro_f1']:.3f} occ_macro_f1={metrics['occ_macro_f1']:.3f}"
    )
...
def main():
    ...
    train_indices, val_indices = split_preencoded_by_episode(dataset, args.val_ratio, args.seed)
