...
def assign_shortage_routes(obs, hire_count):
    ...
    replant_tasks = [task for task in remaining if task[0] == "REPLANT_WHEAT"]
    for task in replant_tasks:
        candidate = routes[supplier_worker] + [task]
        if route_cost(supplier_worker, candidate) <= get_worker_capacity(supplier_worker):
            routes[supplier_worker].append(task)
            remaining.remove(task)
