"""CHI 11: public-meta shop routing and market-impact control.

This experimental agent keeps CHI 10's economic planner and adds three
small, observation-only ideas found in high-scoring public Kaggriculture
agents:

* the first unlocked shop provides a mild production prior;
* repeated public farm similarity enables clone-aware sell preemption;
* existing SELL slots are ordered by their estimated price impact.

No opponent identity, episode identifier, replay lookup, or future action is
used.  

No project-local imports are required; submit this file directly.
"""
...
def get_expected_production_profit(obs, production, opponent_supply=None, days_ahead=1):
    ...
    if opponent_supply is None:
        opponent_supply = {}
        for goods in public_supply_events(obs, 1 - obs['player'], min(29, obs['day'] + int(days_ahead))).values():
            for product, quantity in goods.items():
                opponent_supply[product] = opponent_supply.get(product, 0) + quantity
    revenue, prices = 0, {}
    for product, quantity in production.items():
        if quantity <= 0 or product not in MARKET_PARAMS:
            continue
        inventory = obs['market']['inventory'][product] - consumption.get(product, 0) + opponent_supply.get(product, 0)
        value, _ = sell_units(product, inventory, quantity)
        revenue += value
        prices[product] = value / quantity
    return revenue, prices
...
def get_marginal_expected_revenue(obs, candidate_production, occupation_time):
    day = min(LAST_SELL_DAY, obs['day'] + int(occupation_time))
    return marginal_event_revenue(obs, {day: candidate_production}, day)
...
def get_task_expected_production(task):
    ...
    if not isinstance(tile, dict):
        return {}
    if kind in ('CROP', 'WHEAT_SUPPLY') and tile.get('kind') == 'PLANT':
        crop = tile['crop']
        ...
        return {crop: held}
    if kind in ('ANIMAL', 'ANIMAL_PRE', 'ANIMAL_FEED') and tile.get('animal'):
...
def get_worker_capacity(worker_id):
    end = FINAL_DROP_HOUR + 1 if CURRENT_DAY == LAST_SELL_DAY else TURNS_PER_DAY
    return max(0, end - (WORKER_HIRE_TICKS.get(worker_id, 0) + 1 if worker_id else 0))
...
def _uncached_task_action_cost(task):
    kind = task[0]
    if kind == "WAIT_WHEAT":
        return 0
    if kind == "SHED_DROP":
...
def task_action_cost(task):
    key = ("task_action_cost", task)
    if key not in ROUTE_CACHE:
        ROUTE_CACHE[key] = _uncached_task_action_cost(task)
...
def _uncached_order_tasks(start, tasks):
    ordered = []
...
def order_tasks(start, tasks):
    key = ("order_tasks", (start, tuple(tasks)))
    if key not in ROUTE_CACHE:
        ROUTE_CACHE[key] = _uncached_order_tasks(start, tasks)
    value = ROUTE_CACHE[key]
    return value.copy() if isinstance(value, list) else value
...
def _uncached_route_cost(worker_id, route):
    start = get_worker_spawn(worker_id)
    cost = sequence_cost(start, route) + len(get_animal_pickups(route))
    if get_feed_count(route) and not route_has_wait(route):
        cost += 1
    if CURRENT_DAY == LAST_SELL_DAY:
        cost += distance_to_shed(route_end(start, route)) + 1
    elif CURRENT_OBS and route_needs_midday_drop(route):
        # Reserve a conservative round trip and a product transfer for a mid-day sale.
        farthest = max((distance_to_shed(t[1]) for t in route), default=0)
        cost += 2 * farthest + 2
    return cost


def route_cost(worker_id, route):
    key = ("route_cost", (worker_id, tuple(route)))
    if key not in ROUTE_CACHE:
        ROUTE_CACHE[key] = _uncached_route_cost(worker_id, route)
    value = ROUTE_CACHE[key]
    return value.copy() if isinstance(value, list) else value
...
def assign_normal_routes(obs, hire_count, include_animals=True, required_installs=None):
    ...
    for task in critical:
        if not add_task_to_routes(routes, task):
            return None
    crop_tasks = [("CROP", pos) for pos, data in CROP_STATE.items() if (data["planted"] or data.get("weed", False) or data.get("next_prod") or not EXPANSION_ACTIVE) and max(0, crop_action_cost(pos) - int(crop_needs_water(pos))) > 0]
    ...
    tasks.sort(key=lambda task: (-get_task_profitability(obs, task), distance(CENTER, task[1]), task[1][1], task[1][0]))
    for task in tasks:
...
def get_emergency_drop_action(obs, worker_id):
    ...
    if pos is None or worker_id >= len(inventories):
        return None
    state = WORKER_STATE[worker_id]
    inv = inventories[worker_id]
    # Keep feed and animals with the worker. PLACE transfers only the requested product.
    goods = {p: n for p, n in inv.items() if p in MARKET_PARAMS and p != 'WHEAT' and n > 0}
    if not goods:
        state['dropping'] = False
        return None
    if not state.get('dropping') and get_total_nonseed_stock(obs) < 90:
        return None
    state['dropping'] = True
    pos = tuple(pos)
    ...
    if DROP_ROOM <= 0:
        return ['PASS']
    product = max(goods, key=lambda p: (goods[p], obs['market']['prices'].get(p, 0)))
    quantity = min(goods[product], DROP_ROOM)
    DROP_ROOM -= quantity
    TURN_TRANSFERS[product] = TURN_TRANSFERS.get(product, 0) + quantity
    return ['PLACE', product, quantity]


def get_general_worker_action(obs, worker_id):
    state = WORKER_STATE[worker_id]
    route = WORKER_ROUTES.get(worker_id, [])
    position = get_worker_position(obs, worker_id)
    if position is None:
        return ['PASS']
    pos = tuple(position)
    # Complete a fresh planting's mandatory water before any diversion.
    if state['stack'] and state['stack'][-1][0] == 'WATER' and state['target'] < len(route) and pos == route[state['target']][1]:
        tile = obs['farms'][obs['player']]['tiles'][pos[1]][pos[0]]
        if isinstance(tile, dict) and tile.get('kind') == 'PLANT' and not tile.get('watered_today') and tile.get('consecutive_unwatered', 0) >= 1:
            return state['stack'].pop()
    emergency = get_emergency_drop_action(obs, worker_id)
    if emergency is not None:
        return emergency
    if state.get('refill'):
        target = nearest_shed_tile(pos)
        if get_worker_wheat(obs, worker_id) > 0:
            state['refill'] = False
        elif pos != target:
            return move_toward(pos, target)
        else:
            n = min(max(1, get_feed_count(route[state['target']:])), obs['private']['shed'].get('WHEAT', 0))
            return ['PICKUP', 'WHEAT', n] if n else ['PASS']
    if state['stack'] and state['target'] < len(route):
        task = route[state['target']]
        if pos != task[1]:
            return move_toward(pos, task[1])
        while state['stack']:
            action = state['stack'][-1]
            if not valid_stack_action(obs, worker_id, action, task):
                state['stack'].pop()
                continue
            if action[0] == 'FEED' and get_worker_wheat(obs, worker_id) <= 0:
                # Refill at the shed, then return to this same task.
                target = nearest_shed_tile(pos)
                if pos != target:
                    state['refill'] = True
                    return move_toward(pos, target)
    ...
    if state['animal_pickups'] or (not state['pickup_done'] and get_feed_count(route)):
        target = nearest_shed_tile(pos)
        if pos != target:
            return move_toward(pos, target)
        if state['animal_pickups']:
            animal, n = state['animal_pickups'][0]
            have = obs['private']['inventories'][worker_id].get(animal, 0)
            if have >= n:
                state['animal_pickups'].pop(0)
            else:
                available = obs['private']['shed'].get(animal, 0)
                return ['PICKUP', animal, min(n-have, available)] if available else ['PASS']
            if state['animal_pickups']:
                return ['PASS']
        n = get_feed_count(route) - get_worker_wheat(obs, worker_id)
        if n > 0:
            available = obs['private']['shed'].get('WHEAT', 0)
            return ['PICKUP', 'WHEAT', min(n, available)] if available else ['PASS']
        state['pickup_done'] = True
    task = route[state['target']]
    if pos != task[1]:
        return move_toward(pos, task[1])
    if obs['farms'][obs['player']]['tiles'][pos[1]][pos[0]] == 'LOCKED':
        return ['PASS']
    actions = get_task_actions(obs, task)
    if CURRENT_DAY == LAST_SELL_DAY:
        actions = [a for a in actions if a[0] in ('HARVEST', 'COLLECT_FERTILIZER', 'WATER')]
    if not actions:
        state['target'] += 1
        return ['PASS']
    state['active_task'] = True
    state['stack'] = list(reversed(actions))
    if valid_stack_action(obs, worker_id, state['stack'][-1], task):
        return state['stack'].pop()
    state['stack'].pop()
    return ['PASS']
...
def get_final_drop_action(obs, worker_id):
    ...
    target = nearest_shed_tile(pos)
    if obs['hour'] + distance(pos, target) < FINAL_DROP_HOUR:
        return None
    if pos != target:
        return move_toward(pos, target)
    quantity = sum(inv.values())
    if quantity <= DROP_ROOM:
        DROP_ROOM -= quantity
        for product, count in inv.items():
            TURN_TRANSFERS[product] = TURN_TRANSFERS.get(product, 0) + count
        return ['DROP'] if quantity else ['PASS']
    goods = [p for p, n in inv.items() if p in MARKET_PARAMS and n > 0]
    if not goods or DROP_ROOM <= 0:
        return ['PASS']
    product = max(goods, key=lambda p: obs['market']['prices'].get(p, 0))
    quantity = min(inv[product], DROP_ROOM)
    DROP_ROOM -= quantity
    TURN_TRANSFERS[product] = TURN_TRANSFERS.get(product, 0) + quantity
    return ['PLACE', product, quantity]
...
def begin_turn(obs):
    global DROP_ROOM, TURN_CACHE, CURRENT_DAY
    TURN_CACHE.clear()
    TURN_TRANSFERS.clear()
    ROUTE_CACHE.clear()
    DROP_ROOM = max(0, 100 - sum(obs['private']['shed'].values()))
...
def valid_stack_action(obs, worker_id, action, task):
    ...
    if op == 'HARVEST':
        return isinstance(tile, dict) and (tile.get('yield_units', 0) > 0 or (tile.get('kind') == 'PLANT' and CROP_PRODUCTION[tile['crop']]['type'] == 'ONE_TIME' and CROP_STATE.get(pos, {}).get('harvest')))
    if op in ('FEED', 'CARE', 'COLLECT_FERTILIZER'):
        ...
        return bool(tile.get(field)) if op == 'COLLECT_FERTILIZER' else not tile.get(field, False)
    if op.startswith('BUILD_'):
        return tile is None
    if op == 'PLACE':
        return isinstance(tile, dict) and tile.get('kind') == ('COOP' if action[1] == 'GOOSE' else 'PASTURE') and not tile.get('animal')
    return True
...
def route_needs_midday_drop(route):
    obs = CURRENT_OBS
    key = ('retained_stock',)
    if key not in TURN_CACHE:
        sold = sum(order[2] for order in adaptive_sale_orders(obs))
        TURN_CACHE[key] = max(0, get_total_nonseed_stock(obs) - sold)
    produced = sum(sum(get_task_expected_production(t).values()) for t in route)
    # Immediate market sales free yesterday's stock before workers start harvesting.
    # Do not charge every worker for a hypothetical trip caused by already-sold goods.
    share = max(8, (90 - TURN_CACHE[key]) / max(1, HIRE_COUNT + 1))
    return produced > share
...
# === CHI 11 PUBLIC-META OVERLAY ===

PRODUCT_FOR_PRODUCTION = {
    "WHEAT": "WHEAT",
    "CARROT": "CARROT",
    "TOMATO": "TOMATO",
...
# The prior is deliberately small: CHI 10's marginal-profit calculation
# remains decisive unless two candidates are already close.
FIRST_SHOP_BONUS = 1.08
SECOND_SHOP_BONUS = 1.03
CLONE_DETECTION_START = 48
CLONE_STREAK_REQUIRED = 24
CLONE_DISTANCE_THRESHOLD = 2
CLONE_MAXIMUM_BATCH = 10
CLONE_ACTIVE_START = 160

_META_STATE = {
    0: {"last_step": -1, "clone_streak": 0},
    1: {"last_step": -1, "clone_streak": 0},
}
...
def _shops(obs):
    ...
    return [str(shop) for shop in (_get(town, "unlocked_shops", []) or [])]


def _production_multiplier(obs, production):
    """Return a conservative demand prior derived from public shop order."""
    product = PRODUCT_FOR_PRODUCTION.get(production)
    if product is None:
        return 1.0
    shops = _shops(obs)
    multiplier = 1.0
    if shops and product in SHOP_PRODUCTS.get(shops[0], ()):
        multiplier *= FIRST_SHOP_BONUS
    if len(shops) > 1 and product in SHOP_PRODUCTS.get(shops[1], ()):
        multiplier *= SECOND_SHOP_BONUS
    return multiplier


def _asset_signature(obs, player):
    """Summarize only the opponent information visible in the observation."""
    farms = _get(obs, "farms", []) or []
    if player >= len(farms):
        return ()
    farm = farms[player]
    counts = {name: 0 for name in PRODUCT_FOR_PRODUCTION}
    for row in (_get(farm, "tiles", []) or []):
        for tile in row:
            ...
            if name in counts:
                counts[name] += 1
    quadrants = len(_get(farm, "unlocked_quadrants", ["NW"]) or ["NW"])
    hands = len(_get(farm, "hands", []) or [])
    return tuple(counts[name] for name in sorted(counts)) + (quadrants, hands)


def _clone_active(obs):
    seat = int(_get(obs, "player", 0) or 0)
    step = int(_get(obs, "step", 0) or 0)
    state = _META_STATE[seat]
    if step == 0 or step <= state["last_step"]:
        state["clone_streak"] = 0
    state["last_step"] = step
    own = _asset_signature(obs, seat)
    rival = _asset_signature(obs, 1 - seat)
    distance = sum(abs(a - b) for a, b in zip(own, rival)) if own and rival else 999
    if step >= CLONE_DETECTION_START and distance <= CLONE_DISTANCE_THRESHOLD:
        state["clone_streak"] += 1
    else:
        state["clone_streak"] = 0
    return step >= CLONE_ACTIVE_START and state["clone_streak"] >= CLONE_STREAK_REQUIRED
...
def market_impact(obs, order):
    """Estimate the value lost if this sale executes after its own quantity."""
    if not _is_sell(order):
        return float("-inf")
    product = order[1]
    quantity = max(0, int(order[2]))
    inventory = int(obs["market"]["inventory"].get(product, 10000))
    current = float(obs["market"]["prices"].get(product, unit_price(product, inventory)))
    later = float(unit_price(product, inventory + quantity))
    return quantity * max(0.0, current - later)


def prioritize_sell_slots(obs, action, clone_active=False):
    """Reorder only existing SELL slots, preserving all other order positions."""
    result = deepcopy(action)
    market = list(result.get("market", []))
    # Never strand terminal inventory merely to reduce collision exposure.
    clone_cap = clone_active and int(_get(obs, "day", 0) or 0) < LAST_SELL_DAY
    ranked = []
    for index, order in enumerate(market):
        ...
        if clone_cap:
            candidate[2] = min(int(candidate[2]), CLONE_MAXIMUM_BATCH)
        ranked.append((market_impact(obs, candidate), -index, candidate))
    if len(ranked) < 2:
        if ranked:
            market[next(i for i, order in enumerate(market) if _is_sell(order))] = ranked[0][2]
        result["market"] = market
        return result
    ranked.sort(reverse=True)
    iterator = iter(row[2] for row in ranked)
    result["market"] = [next(iterator) if _is_sell(order) else order for order in market]
    return result
...
def agent(obs):
    """Run the embedded CHI 10 once, with the CHI 11 public-meta overlay."""
    global calculate_production_score

    clone_active = _clone_active(obs)
    original_score = calculate_production_score

    def meta_score(local_obs, production, position):
        score = original_score(local_obs, production, position)
        if score == float("-inf"):
            return score
        return score * _production_multiplier(local_obs, production)

    # Embedded CHI 10 resolves this function through module globals.
    # Restore it immediately after the single base-agent call.
    calculate_production_score = meta_score
    try:
        action = _CHI10_AGENT(obs)
    finally:
        calculate_production_score = original_score

    return prioritize_sell_slots(obs, action, clone_active)
