...
# Ye1: bounded engineering improvements over the exact public V58 artifact.
import copy as _ye1_copy
import dataclasses as _ye1_dataclasses
import math as _ye1_math
...
_YE1_BUILD_INDEX = 0
_YE1_CURRENT_OBS = None
_YE1_CURRENT_CONFIG = None

_YE1_RESIDUAL_MODULE = sys.modules["v49.residual_controller"]
_YE1_SIMULATOR_MODULE = sys.modules["v23.simulator"]
_YE1_POLICY_MODULE = sys.modules["v23.policy_library"]
_YE1_IMPACT_MODULE = sys.modules["scripts.v22_market_impact"]
_YE1_MARKET_MAKER_MODULE = sys.modules["v24.market_maker"]
_YE1_TERMINAL_MODULE = sys.modules["scripts.v19_terminal"]
_YE1_ORIGINAL_MARKET_PRICE = _YE1_SIMULATOR_MODULE.market_price
_YE1_ORIGINAL_MARKET_MAKER = _YE1_RESIDUAL_MODULE.MarketMakerExpert


def _ye1_get(value, key, default=None):
    if isinstance(value, dict):
...
class _Ye1DisabledMarketMaker:
    def apply(self, _obs, action, _configuration=None):
        return action


class _Ye1MarketMakerFactory:
    def __new__(cls, actions, config):
        if _YE1_FEATURES.get("lazy_disabled") and not bool(config.enabled):
            return _Ye1DisabledMarketMaker()
        return _YE1_ORIGINAL_MARKET_MAKER(actions, config)


_YE1_RESIDUAL_MODULE.MarketMakerExpert = _Ye1MarketMakerFactory


def _ye1_shape(name, value, scale=None):
    value = max(0.0, float(value))
    if name == "sq":
        return value * value
    if name == "sqrt":
        return _ye1_math.sqrt(value)
    if name == "log":
        return _ye1_math.log1p(value)
    if name == "log10":
        return _ye1_math.log10(1.0 + value)
...
def _ye1_market_price(item, inventory):
    if not _YE1_FEATURES.get("effective_market"):
        return _YE1_ORIGINAL_MARKET_PRICE(item, inventory)
    fields = (
        "base", "I0", "T", "below_func", "below_target",
        "above_func", "above_target",
    )
    params = dict(zip(fields, _YE1_SIMULATOR_MODULE.MARKET_PARAMS[item]))
    for overrides in (
        _ye1_get(_YE1_CURRENT_CONFIG, "marketParams", {}) or {},
        _ye1_get(_ye1_get(_YE1_CURRENT_OBS, "market", {}) or {}, "params", {}) or {},
    ):
        patch = _ye1_get(overrides, item, {})
        if isinstance(patch, dict):
            params.update(patch)
    side = "below" if int(inventory) < params["I0"] else "above"
    shape = params[side + "_func"]
    amplitude = (
        params[side + "_target"] * params["base"]
        / _ye1_shape(shape, params["T"], params["T"])
    )
    change = amplitude * _ye1_shape(
        shape, abs(int(inventory) - params["I0"]), params["T"]
    )
    price = params["base"] + (change if side == "below" else -change)
    return max(1, int(round(price)))


for _ye1_module in (
    _YE1_SIMULATOR_MODULE,
    _YE1_POLICY_MODULE,
    _YE1_IMPACT_MODULE,
    _YE1_MARKET_MAKER_MODULE,
):
    if "market_price" in vars(_ye1_module):
        _ye1_module.market_price = _ye1_market_price


def _ye1_build_policy(route, config):
    if _YE1_FEATURES.get("terminal_parity"):
        config = _ye1_dataclasses.replace(config, terminal_rule="none")
    return _v51_build_policy(route, config)


def _ye1_route_fallback(name, obs):
    step = int((obs or {}).get("step", 0) or 0)
    routes = dict(_V58_POLICY_ROUTES)
    route = routes.get(name) or routes["base_backbone"]
    action = _ye1_copy.deepcopy(route[min(max(0, step), len(route) - 1)])
    return _YE1_TERMINAL_MODULE.align_hands(action, obs)


def _ye1_build_next_policy():
    global _YE1_BUILD_INDEX
    if _YE1_BUILD_INDEX >= len(_V58_POLICY_ROUTES):
        return
    name, route = _V58_POLICY_ROUTES[_YE1_BUILD_INDEX]
    _YE1_BUILD_INDEX += 1
    policy = _ye1_build_policy(route, _V54_CONFIG)
    for previous_obs, previous_visible in _V58_WARM_HISTORY[:-1]:
        policy(previous_obs, previous_visible)
    _V58_POLICIES[name] = policy


def _ye1_required_names(obs, state):
    names = [name for name, _route in _V58_POLICY_ROUTES]
    if not _YE1_FEATURES.get("selected_only"):
        return names
    step = int((obs or {}).get("step", 0) or 0)
    if step < 72:
        return names
...
def _ye1_advance_policies(obs, visible, state):
    global _YE1_CURRENT_OBS, _YE1_CURRENT_CONFIG
    _YE1_CURRENT_OBS, _YE1_CURRENT_CONFIG = obs, visible
    if _YE1_BUILD_INDEX < len(_V58_POLICY_ROUTES):
        _V58_WARM_HISTORY.append(
            (_ye1_copy.deepcopy(obs), _ye1_copy.deepcopy(visible))
        )
        _ye1_build_next_policy()
    required = set(_ye1_required_names(obs, state))
    actions = {}
    called = {}
    for name, _route in _V58_POLICY_ROUTES:
        policy = _V58_POLICIES.get(name)
        if policy is None or name not in required:
            continue
        policy_key = id(policy)
        if policy_key in called:
            actions[name] = called[policy_key]
            continue
        if _YE1_FEATURES.get("fault_isolation"):
            try:
                action = policy(obs, visible)
            except Exception:
                errors = _YE1_TELEMETRY["policy_errors"]
                errors[name] = int(errors.get(name, 0)) + 1
                _YE1_TELEMETRY["fallbacks"] += 1
                action = _ye1_route_fallback(name, obs)
        else:
            action = policy(obs, visible)
        called[policy_key] = action
        actions[name] = action
        _YE1_TELEMETRY["policy_calls"] += 1
    if _YE1_BUILD_INDEX >= len(_V58_POLICY_ROUTES):
        _V58_WARM_HISTORY.clear()
    return actions


def _ye1_terminal_action(obs, action, configuration):
    capacity = max(1, int(_ye1_get(configuration, "shedCapacity", 100) or 100))
    maximum = max(1, int(_ye1_get(configuration, "maxMarketOrdersPerTurn", 10) or 10))
    shed, _inventories = _YE1_MARKET_MAKER_MODULE._project_private_after_actor(
        obs, action, shed_capacity=capacity
    )
    prices = _ye1_get(_ye1_get(obs, "market", {}) or {}, "prices", {}) or {}
    exposure = _YE1_TERMINAL_MODULE.opponent_exposure(obs)
    rows = []
    for index, item in enumerate(_YE1_TERMINAL_MODULE.SELLABLE):
        quantity = max(0, int(shed.get(item, 0) or 0))
        if quantity <= 0:
            continue
        price = max(1.0, float(_ye1_get(prices, item, 1) or 1))
        score = (
            (1.0 + exposure.get(item, 0.0))
            * _YE1_TERMINAL_MODULE.GLUT_WEIGHT.get(item, 1.0)
            * price
            * _ye1_math.log1p(quantity)
        )
        rows.append((score, -index, item, quantity))
    rows.sort(reverse=True)
    result = _YE1_TERMINAL_MODULE.align_hands(action, obs)
    result["market"] = [
        ["SELL", item, quantity]
        for _score, _index, item, quantity in rows[:maximum]
    ]
    return result


def _ye1_reset_runtime(features=None):
    global _YE1_BUILD_INDEX, _YE1_CURRENT_OBS, _YE1_CURRENT_CONFIG
    if features is not None:
        _YE1_FEATURES.update(features)
    _V58_POLICIES.clear()
    _V58_WARM_HISTORY.clear()
    _V58_STATES.clear()
    _V58_STATES.update({0: {}, 1: {}})
    _YE1_BUILD_INDEX = 0
    _YE1_CURRENT_OBS = None
    _YE1_CURRENT_CONFIG = None
    _YE1_TELEMETRY.update({
        "calls": 0,
        "policy_calls": 0,
        "policy_errors": {},
        "outer_errors": 0,
...
def _jet3_nearest_shed(position, configuration):
    half = max(1, int(_ye1_get(configuration, "boardSize", 10) or 10) // 2)
    tiles = (
    ...
    return min(
        tiles,
        key=lambda tile: (
            abs(point[0] - tile[0]) + abs(point[1] - tile[1]),
            tile[1], tile[0],
        ),
    )


def _jet3_move_toward(position, target):
...
def _jet3_final_drop(obs, action, configuration, final_step):
    """CHI11-style just-in-time return of carried stock for liquidation."""
    step = int((obs or {}).get("step", 0) or 0)
    remaining = final_step - step
    if remaining < 0:
        return action
    seat = _v58_seat(obs)
    farm = _v58_farms(obs)[seat]
    positions = [farm.get("farmer"), *(farm.get("hands", []) or [])]
    private = (obs or {}).get("private", {}) or {}
    inventories = list(private.get("inventories", []) or [])
    result = _ye1_copy.deepcopy(action or {})
    actor_actions = [
        list(result.get("farmer") or ["PASS"]),
        *[list(value or ["PASS"]) for value in (result.get("hands") or [])],
    ]
    ...
    for index, (position, inventory) in enumerate(zip(positions, inventories)):
        ...
        if not any(int(value or 0) > 0 for value in inventory.values()):
            continue
        target = _jet3_nearest_shed(position, configuration)
        distance = abs(int(position[0]) - target[0]) + abs(int(position[1]) - target[1])
        if remaining > distance + 1:
            continue
        actor_actions[index] = ["DROP"] if distance == 0 else _jet3_move_toward(position, target)
    result["farmer"] = actor_actions[0]
    result["hands"] = actor_actions[1:len(positions)]
    return result


def _jet3_filter_dead_seed_buys(obs, action):
    """Do not buy a seed that the selected continuation never plants."""
    step = int((obs or {}).get("step", 0) or 0)
    result = _ye1_copy.deepcopy(action or {})
    ...
    if not crops:
        return result
    future = {}
    for route_action in _V58_KNOWN_YARN_ROUTE[step + 1:]:
        actors = [
            route_action.get("farmer") or ["PASS"],
            *(route_action.get("hands") or []),
        ]
        for actor_action in actors:
            if len(actor_action) >= 2 and actor_action[0] == "PLANT":
                crop = str(actor_action[1])
                future[crop] = future.get(crop, 0) + 1
    seeds = _ye1_get(_ye1_get(obs, "private", {}) or {}, "seeds", {}) or {}
    filtered = []
    for order in orders:
        if len(order) < 3 or order[0] != "BUY_SEED":
            ...
            continue
        crop = str(order[1])
        needed = max(0, int(future.get(crop, 0)) - int(_ye1_get(seeds, crop, 0) or 0))
        quantity = min(max(0, int(order[2])), needed)
        if quantity > 0:
            filtered.append([order[0], order[1], quantity])
    result["market"] = filtered
    return result


_JET5_FERTILIZER_TELEMETRY = {
    "buys_examined": 0,
    "buys_modified": 0,
    "buys_removed": 0,
    "units_avoided": 0,
...
def _jet5_count_fertilize(actions):
    count = 0
    for route_action in actions:
        actors = [
            route_action.get("farmer") or ["PASS"],
            *(route_action.get("hands") or []),
        ]
        count += sum(
            1 for actor_action in actors
            if actor_action and actor_action[0] == "FERTILIZE"
        )
    return count


def _jet5_future_fertilize_need(step):
    """Count planned one-unit FERTILIZE uses in Jet3's selected continuation."""
    routes = dict(_V58_POLICY_ROUTES)
    known_yarn = routes["known_yarn"]
    if step >= 72:
        return _jet5_count_fertilize(known_yarn[step + 1:])
    backbone = routes["base_backbone"]
    return (
        _jet5_count_fertilize(backbone[step + 1:72])
        + _jet5_count_fertilize(known_yarn[72:])
    )


def _jet5_observed_fertilizer(obs):
    """Return shed plus carried fertilizer, or None when observation is incomplete."""
    private = (obs or {}).get("private")
    ...
    if any(not isinstance(inventory, dict) for inventory in inventories):
        return None
    try:
        return max(0, int(shed.get("FERTILIZER", 0) or 0)) + sum(
            max(0, int(inventory.get("FERTILIZER", 0) or 0))
...
def _jet5_is_fertilizer_buy(order):
    return (
...
def _jet5_filter_dead_fertilizer_buys(obs, action):
    """Cap fertilizer purchases at the selected continuation's remaining need."""
    result = _ye1_copy.deepcopy(action or {})
    orders = list(result.get("market", []) or [])
    targets = [order for order in orders if _jet5_is_fertilizer_buy(order)]
    if not targets:
        return result
    _JET5_FERTILIZER_TELEMETRY["buys_examined"] += len(targets)

    available = _jet5_observed_fertilizer(obs)
    if available is None:
        return result
    step = int((obs or {}).get("step", 0) or 0)
    current_uses = _jet5_count_fertilize([result])
    available_after_current = max(0, available - current_uses)
    useful_purchase = max(
        0,
        _jet5_future_fertilize_need(step) - available_after_current,
    )

    kept_quantity = 0
    filtered = []
    for order in orders:
        if not _jet5_is_fertilizer_buy(order):
            filtered.append(order)
            continue
        try:
        ...
        if original <= 0:
            filtered.append(order)
            continue
        quantity = min(original, max(0, useful_purchase - kept_quantity))
        avoided = original - quantity
        if avoided > 0:
            _JET5_FERTILIZER_TELEMETRY["buys_modified"] += 1
            _JET5_FERTILIZER_TELEMETRY["units_avoided"] += avoided
        if quantity <= 0:
            _JET5_FERTILIZER_TELEMETRY["buys_removed"] += 1
            continue
        revised = list(order)
        revised[2] = quantity
        filtered.append(revised)
        kept_quantity += quantity
    result["market"] = filtered
    return result


_JET6_FERTILIZER_RECOVERY_TELEMETRY = {
    "dead_collects_examined": 0,
    "collection_routes_started": 0,
    "collections_advanced": 0,
    "fertilizer_units_recovered": 0,
}
_JET6_EARLY_SALE_TELEMETRY = {
    "opportunities": 0,
    "orders_added": 0,
    "units_offered": 0,
    "full_market_skips": 0,
...
def _jet6_sell_available_products(obs, action, configuration):
    """Use otherwise-idle market slots to monetize finished products early."""
    private = (obs or {}).get("private")
    ...
    maximum = max(
        1,
        int(_ye1_get(configuration, "maxMarketOrdersPerTurn", 10) or 10),
    )
    sold = {}
    for order in orders:
    ...
    candidates = []
    for item in _YE1_TERMINAL_MODULE.SELLABLE:
        if item in {"WHEAT", "FERTILIZER"}:
            continue
        quantity = max(
            0,
            int(private["shed"].get(item, 0) or 0) - sold.get(item, 0),
        )
        if quantity > 0:
            candidates.append((item, quantity))
    if not candidates:
        return result
    _JET6_EARLY_SALE_TELEMETRY["opportunities"] += 1
    slots = max(0, maximum - len(orders))
    if slots == 0:
        _JET6_EARLY_SALE_TELEMETRY["full_market_skips"] += 1
        return result
    for item, quantity in candidates[:slots]:
        orders.append(["SELL", item, quantity])
        _JET6_EARLY_SALE_TELEMETRY["orders_added"] += 1
        _JET6_EARLY_SALE_TELEMETRY["units_offered"] += quantity
    result["market"] = orders
    return result


def _jet6_collectable_fertilizer(tile):
    return (
...
def _jet6_recover_terminal_fertilizer(obs, action, configuration):
    """Pull one terminal fertilizer collection forward so it can be sold."""
    result = _ye1_copy.deepcopy(action or {})
    if not isinstance(obs, dict) or configuration is None or "step" not in obs:
        return result
    try:
        step = int(obs["step"])
        final_step = int(_ye1_get(configuration, "episodeSteps")) - 2
    except (TypeError, ValueError):
        return result
    if step not in (final_step - 4, final_step - 3):
        return result
    ...
    tiles = farm.get("tiles")
    if not isinstance(inventories, list) or not isinstance(tiles, list):
        return result
    ...
    if len(actor_actions) < len(positions) or len(inventories) < len(positions):
        return result

    for index, (position, inventory) in enumerate(zip(positions, inventories)):
        if not isinstance(position, (list, tuple)) or len(position) < 2:
            continue
        if not isinstance(inventory, dict) or any(int(value or 0) > 0 for value in inventory.values()):
            continue
        try:
            x, y = int(position[0]), int(position[1])
            tile = tiles[y][x]
        ...
        if step == final_step - 4:
            if actor_actions[index][0] != "COLLECT_FERTILIZER":
                continue
            _JET6_FERTILIZER_RECOVERY_TELEMETRY["dead_collects_examined"] += 1
            if _jet6_collectable_fertilizer(tile):
                continue
            candidates = []
            for tx, ty in ((x - 1, y), (x + 1, y), (x, y - 1), (x, y + 1)):
                try:
                    target_tile = tiles[ty][tx]
                except (IndexError, TypeError):
                    continue
                if tx < 0 or ty < 0 or not _jet6_collectable_fertilizer(target_tile):
                    continue
                shed = _jet3_nearest_shed((tx, ty), configuration)
                distance = abs(tx - shed[0]) + abs(ty - shed[1])
                if step + 2 + distance <= final_step:
                    candidates.append((distance, ty, tx))
            if not candidates:
                continue
            _distance, ty, tx = min(candidates)
            actor_actions[index] = _jet3_move_toward((x, y), (tx, ty))
            _JET6_FERTILIZER_RECOVERY_TELEMETRY["collection_routes_started"] += 1
        elif (
            actor_actions[index][0] == "CARE"
            and _jet6_collectable_fertilizer(tile)
        ):
            shed = _jet3_nearest_shed((x, y), configuration)
            distance = abs(x - shed[0]) + abs(y - shed[1])
            if step + 1 + distance <= final_step:
                actor_actions[index] = ["COLLECT_FERTILIZER"]
                telemetry = _JET6_FERTILIZER_RECOVERY_TELEMETRY
                telemetry["collections_advanced"] += 1
...
def agent(obs, configuration=None):
    try:
        ...
        actions = _ye1_advance_policies(obs, visible, state)
        base_action = _v58_base_action(obs, step, shops, actions)
        mode = str(state.get("mode") or "base")
        name = "known_yarn" if step >= 72 else None
        result = actions.get(name, base_action) if name else base_action
        episode_steps = max(2, int(_ye1_get(configuration, "episodeSteps", 720) or 720))
        result = _jet3_final_drop(obs, result, visible, episode_steps - 2)
        result = _jet3_filter_dead_seed_buys(obs, result)
        if _YE1_FEATURES.get("terminal_parity") and step == episode_steps - 2:
            result = _ye1_terminal_action(obs, result, visible)
        result = _jet5_filter_dead_fertilizer_buys(obs, result)
        result = _jet6_recover_terminal_fertilizer(obs, result, visible)
        result = _jet6_sell_available_products(obs, result, visible)
        state["last_step"] = step
        _YE1_TELEMETRY["calls"] += 1
        return result
    except Exception:
        _YE1_TELEMETRY["outer_errors"] += 1
        _YE1_TELEMETRY["fallbacks"] += 1
        return _v51_safe_action(obs)
...
def kaggle_agent_jet6(obs, configuration=None):
    return agent(obs, configuration)
