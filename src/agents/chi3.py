...
def build_worker_route(worker_id, available_tasks):
    route = []
    remaining = available_tasks.copy()
    current_pos = get_worker_spawn(worker_id)

    while remaining:
        ordered = sorted(remaining, key=lambda task: (distance(current_pos, task[1]), distance(CENTER, task[1]), -task_action_cost(task)))
        added = False
        for task in ordered:
            candidate = route + [task]
            if route_cost(worker_id, candidate) <= get_worker_capacity(worker_id):
                route.append(task)
