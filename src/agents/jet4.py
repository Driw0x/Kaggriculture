...
_JET4_MIN_EXPECTED_MONEY_PER_WORKER_TICK = 1.0
_JET4_MIN_PRODUCTIVE_TICKS = 1
_JET4_HIRE_TELEMETRY = {
    "hires_examined": 0,
    "hires_modified": 0,
    "workers_avoided": 0,
}
...
def _jet4_fibonacci(index):
    """Return the one-based Fibonacci hire cost before its config multiplier."""
    previous, current = 0, 1
    for _ in range(max(1, int(index))):
        previous, current = current, previous + current
    return previous


def _jet4_filter_late_hires(obs, action, configuration, final_step):
    """Remove only hires whose conservative payback exceeds the game horizon."""
    result = _ye1_copy.deepcopy(action or {})
    orders = list(result.get("market", []) or [])
    if not any(order and order[0] == "HIRE" for order in orders):
        return result

    step = int((obs or {}).get("step", 0) or 0)
    remaining_ticks = max(0, int(final_step) - step)
    seat = _v58_seat(obs)
    farm = _v58_farms(obs)[seat]
    hires_today = max(0, int(farm.get("hires_today", 0) or 0))
    cost_multiplier = max(
        0,
        int(_ye1_get(configuration, "farmHandCostMult", 1) or 0),
    )
    kept_hires = 0
    filtered = []
    for order in orders:
        if not order or order[0] != "HIRE":
            filtered.append(order)
            continue
        _JET4_HIRE_TELEMETRY["hires_examined"] += 1
        hire_index = hires_today + kept_hires + 1
        hire_cost = cost_multiplier * _jet4_fibonacci(hire_index)
        payback_ticks = max(
            _JET4_MIN_PRODUCTIVE_TICKS,
            int(_ye1_math.ceil(
                hire_cost / _JET4_MIN_EXPECTED_MONEY_PER_WORKER_TICK
            )),
        )
        if remaining_ticks < payback_ticks:
            _JET4_HIRE_TELEMETRY["hires_modified"] += 1
            _JET4_HIRE_TELEMETRY["workers_avoided"] += 1
            continue
        filtered.append(order)
        kept_hires += 1
    result["market"] = filtered
...
def agent(obs, configuration=None):
    try:
        ...
        result = _jet4_filter_late_hires(
            obs, result, visible, episode_steps - 2
        )
        state["last_step"] = step
...
def kaggle_agent_jet4(obs, configuration=None):
    return agent(obs, configuration)
