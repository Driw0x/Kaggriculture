...
QUANTITY_OCCURRENCE_INDICES = [OCCURRENCE_NAMES.index(name) for name in QUANTITY_NAMES]
SELL_OCCURRENCE_INDICES = [OCCURRENCE_NAMES.index(name) for name in SELL_NAMES]
...
class BCDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return {
            ...
            "hire": torch.tensor(target["hire"], dtype=torch.long),
            "occurrence": occurrence,
            "quantity": torch.tensor(target["quantity"], dtype=torch.float32),
            "quantity_mask": occurrence[QUANTITY_OCCURRENCE_INDICES],
            "sell_ratio": torch.tensor(target["sell_ratio"], dtype=torch.float32),
            "sell_mask": occurrence[SELL_OCCURRENCE_INDICES],
        }
